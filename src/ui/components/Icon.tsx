/**
 * Thin-line nautical icon set + the compass-rose brand mark, ported from the
 * prototype's icons.jsx. All strokes are currentColor, so an icon takes the
 * surrounding text colour. The path data is copied verbatim from the design
 * source — only the React/TS plumbing (typed `name` union, no window globals)
 * is new.
 */
import type { SVGProps, ReactNode } from 'react';

interface SvgProps extends Omit<SVGProps<SVGSVGElement>, 'stroke'> {
  s?: number;
  sw?: number;
  children: ReactNode;
}

function Svg({ s = 20, sw = 1.6, children, ...rest }: SvgProps): JSX.Element {
  return (
    <svg
      width={s}
      height={s}
...
export function Icon({ name, s = 20, sw = 1.6, ...rest }: IconProps): JSX.Element {
  return (
    <Svg s={s} sw={sw} role="img" aria-label={name} data-icon={name} {...rest}>
      {PATHS[name] ?? PATHS.info}
    </Svg>
  );
}

/** Compass rose — the brand mark + page watermark. */
export function CompassRose({ s = 34, sw = 1.4, ...rest }: Omit<SVGProps<SVGSVGElement>, 'stroke'> & { s?: number; sw?: number }): JSX.Element {
  return (
    <svg
      width={s}
      height={s}
